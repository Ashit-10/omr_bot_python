...
async def rectContour(contours, img2, streo):
    rectCon =[]
    all_cods = []
    ...
    for i in contours :        
        ...
        if len(approx) ==4 and area > 1500 and area < streo:
...
async def read_img(img_file_name, output_file_name_given, ans_key_given, eval_folder, caption_file_name, cap_given, *args):
    ...
    thresh_values = [125, 110, 85, 80, 75, 70, 210, 205, 195, 185, 170, 165, 160, 155, 145, 135]
    ...
    for thr in thresh_values:
        ...
        all_cods = await rectContour(contours, img2, streo)
    ...
    detected_circles = cv2.HoughCircles(gray2, cv2.HOUGH_GRADIENT, 1, 20, param1 = 50,
                param2 = 30, minRadius = 10, maxRadius = 17)
    ...
    for separate_nums in all_separate:    # looping all numbers till 50        
        ...
        for separate_opts in separate_nums[1]:   # looping each options of each number
            if separate_opts[1] > 1200:  # white pixel values
    ...
    for lets_match_ans in selected_options:
        ...
        if len(selected_opts_list) == 1:
            if ans_key.get(lets_match_ans):   # if answer for that question is in the answer key
                for opts in ans_key[lets_match_ans]:
                    if opts in selected_opts_list:
                        opt_num = tell_num(opts)
    ...
    if roll_num_nums:
        cv2.putText(tframe, g_text, (13 + ans_position - text_u, 246 - text_u), cv2.FONT_HERSHEY_SIMPLEX, text_size, (0,0,150), text_thickness, cv2.LINE_AA)
        cv2.putText(tframe, f"_____", (15 + ans_position , 249), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (139,0,0), 2, cv2.LINE_AA)
        cv2.putText(tframe, by_text, (13 + ans_position - text_u, 269 + text_d), cv2.FONT_HERSHEY_SIMPLEX, text_size, (0,0,150), text_thickness, cv2.LINE_AA)
    ...
    selected_num = total_keys - (correct_num + wrong_num)
    return out_put, selected_num, correct_num, wrong_num, total_keys, actual_roll, roll_num_nums  # actual caption rol should be given
